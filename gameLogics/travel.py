...
import random
import json
from gameLogics.utils.custom_json_encoder import CustomJsonEncoder
...
def getMyNextLocation(gameAccount):
    ...
    routeList = json.loads(shipRoute_OBJ.route_details) 
    ...
    if myCurrentLocation in routeList:
        ...
        for i, v in enumerate(MAIN_ROUTE):
            if v == myCurrentLocation and i == len(MAIN_ROUTE) - 1:
                nextLocation = routeList[0]
                break
            elif v == myCurrentLocation:
                if MAIN_ROUTE[i + 1] in routeList:
                    for m, n in enumerate(routeList):
                        if n == MAIN_ROUTE[i + 1]:
                            if m != len(routeList) - 1:
                                nextLocation = routeList[0]
                                break
                            else:
                                nextLocation = routeList[m + 1]
                                break
                else:
                    nextLocation = routeList[0]
                    break
        ...
        if nextLocation:
            nextLocation = nextLocation.strip('[').strip('"')
    return nextLocation

def getDistance(l1,l2):

    dis = Distance.objects.filter(location1=l1,location2=l2).first()
    if not dis:
        dis = Distance.objects.filter(location1=l2,location2=l1).first()
        if not dis and l1 == l2:
    ...
    if dis:
        return dis.distance
    return Distance.objects.first().distance
...
def reduceShipCondition(myGameAccount,weeks):
    ...
    ship_info = json.dumps(ship_info, cls=CustomJsonEncoder)
