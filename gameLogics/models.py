...
class MoneyField(models.DecimalField):
    def from_db_value(self, value, expression, connection):
        if value is not None:
            value = Decimal(str(value).replace('$', '').replace(',', ''))  # Example: Remove $ and commas
        return value
...
class YesNoBooleanField(models.BooleanField):
    def from_db_value(self, value, expression, connection):
        if value in ['Yes', 'yes', 'Y', 'y', 'True', 'true']:
            return True
        elif value in ['No', 'no', 'N', 'n', 'False', 'false']:
            return False
        return None  # Or the default you want for other values
...
class Ships(models.Model):
    ...
    value = MoneyField(max_digits=100, decimal_places=2)
    crew = models.IntegerField()
    cargo = MoneyField(max_digits=100, decimal_places=2)
...
class Crews(models.Model):
    ...
    weeklySalary = MoneyField(max_digits=100, decimal_places=2)
...
class Commodities(models.Model):
    ...
    fragile = YesNoBooleanField()
    perishable = YesNoBooleanField()
    edible = YesNoBooleanField()
...
class GameAccount(models.Model):
    ...
    def cargo_per(self):
        ...
        print(self.ship_info['cargo']['filled'],"ALP",self.ship_info['cargo']['total']*100)

        val = (self.ship_info['cargo']['filled'] / float(self.ship_info['cargo']['total']))*100
        return "%.2f"%val
