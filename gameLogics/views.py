...
def selectShip(request):
    def prepare_dotENV(gameAccount):
        ...
        # Serialize the ship_info using the custom encoder
        com_json_info = json.dumps(com_json_info, cls=CustomJsonEncoder)
        location_json_info = json.dumps(location_json_info, cls=CustomJsonEncoder)
...
def mainGameDashboard(request):
    ...
    locations = Location.objects.filter(name=getMyNextLocation(myGameAccount))

    if locations.exists():
        # Retrieve the first item in the query set
        docking_fee = f'{int(locations[0].docking_fee / 10):,}'
...
def purserNL(request,response):
    ...
    crewSalary = sum([Decimal(c['weeklySalary']) for c in crewMembers])
    final['cwage']['pay'] += crewSalary*charges

    locations = Location.objects.filter(name=new_location)
    if locations.exists():
        # Retrieve the first item in the query set
        docker_fees = int(locations[0].docking_fee / 10)
    else:
        docker_fees = 0
    ...
    print( myGameAccount.ship_info['cwage']['pay'])
    print( myGameAccount.ship_info['cwage']['range'])


    if myGameAccount.ship_info['cwage']['pay'] >  Decimal(myGameAccount.ship_info['cwage']['range']):
    ...
    # Serialize the ship_info using the custom encoder
    final = json.dumps(final, cls=CustomJsonEncoder)
...
def RandR(request):
    ...
    if not isinstance(denv.com_info, dict):
        denv.com_info = json.loads(denv.com_info)
...
def giveRandomCrewmateAndCredit(gameAccount,user,ship_mod):
    ...
    for crews in randomCrewmates:
        ...
        obj['weeklySalary']=str(crews.weeklySalary)
    ...
    ship_info_dict = {
        "ship": {
            ...
            "value" : str(ship_model.value),
            "crew" : str(ship_model.crew),
            "cargo" : str(ship_model.cargo),
        ...
        "cwage":{
            "pay":0,
            "range":str(sum([ Decimal(c['weeklySalary']) for c in clist] )*12),
        },
        "crews":clist,
        "location":{
            "cwar":[]
...
def setCommoditiePrice(location_name):
        ...
        if location_name == "Earth":
            ...
            for com in coms:
                ...
                comlist['available'] = com.deimos.split("|")
                ...
                s,e = priceRange(com.min_price,com.max_price)
                comlist['market_price'] = random.randrange(s,e)
                commodities.append(comlist)
            ...
            for com in coms:
                ...
                comlist['available'] = com.europa.split("|")
...
def getCommoditiePrice(user):
        ...
        if not isinstance(dotenv.com_info, dict):
            dotenv.com_info = json.loads(dotenv.com_info)
